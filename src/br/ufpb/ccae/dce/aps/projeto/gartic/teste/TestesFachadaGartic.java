...
public class TestesFachadaGartic {
   ...
   public void testInitConexaoRede() {
    ...
    public void testNickInvalido() throws ExcecaoGartic {
    	fachada.verificarRede();
 	    fachada.redeConectar();
    	fachada.iniciarJogo();
    	fachada.conexaoBD();
    ...
    @Test(expected=ExcecaoGartic.class)
    public void testInitJogoNaoIniciado() throws ExcecaoGartic {
	    fachada.verificarRede();
	   	fachada.redeConectar();
	   	fachada.conexaoBD();
	   	Assert.assertTrue(fachada.iniciarJogo());
   ...
   @Test (expected=ExcecaoGartic.class)
   public void testJogadorDaVezNaoRecebePalavra() throws ExcecaoGartic{
	   fachada.redeConectar();
   	   fachada.conexaoBD();
	   fachada.iniciarJogo();
	   fachada.insereNick("Carlos");
	   fachada.insereNick("David");
	   Assert.assertTrue(f2.insereNick("Carlos")); // persistencia
   	   Assert.assertTrue(f2.insereNick("David"));
   	   fGUI.initEditor();
	   conectar.intibatePapo();
	   tema.gerarPalavra();
	   tema.palavraAdvinhar();// persistencia
	   fachada.escolherAvezDeJogar("Animal");
	   fachada.palavraRecebida("Animal");
   }
   @Test
